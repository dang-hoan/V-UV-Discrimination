...
function [T_ste, T_ste_sil, T_zcr] = getThreshold(timestamps, label, ste, zcr, frame_duration)
    f = []; g = [];     %ste uv và v
    f1 = []; g1 = [];   %zcr uv và v   
    f2 = [];            %ste sil
    j = 1;
    ...
    %Tìm ngưỡng
    T_ste = threshold(f, g);
    T_ste_sil = threshold(f2, cat(2,f,g));
...
function calAndPlot(wavFile, labFile, frame_duration)
    ...
    [T_ste, T_ste_sil, T_zcr] = getThreshold(timestamps, label, ste, zcr, frame_duration);
    disp("Ngưỡng ste và zcr của audio " + wavFile + ": " + T_ste + ", " + T_zcr);
    disp(T_ste_sil);
    
    %Chuẩn hoá ste và zcr về đoạn [-1;1] (khi đó ngưỡng sẽ đưa về 0)
    T_ste_sil = (T_ste_sil-T_ste)/(T_ste-min(ste));     %Chuẩn hoá ngưỡng T_ste_sil về [-1;1]
    ...
    %Loại bỏ khoảng lặng ảo (chiều dài < 300ms)
    i = 1;
    while i <= len
        sum = 0; j = i;
        if VU(j) == -1
    ...
    for i = 1:len-1
        hold on;
        if VU(i) == -1
            p0 = plot(t(tmp + 1 : tmp + N), x(tmp + 1 : tmp + N),'k');     %silent
        else
            if VU(i) == 1
    ...
    if VU(len) == -1
        plot(t(tmp + 1: tmp + N_rest), x(tmp + 1: tmp + N_rest),'k');     %silent
    else
        if VU(len) == 1
    ...
    legend([p0 p1 p2 p3 p4], {'Silent', 'Voiced', 'Unvoiced', 'My border', 'Standard border'});
    ylim([-1 1]);
    hold off;
